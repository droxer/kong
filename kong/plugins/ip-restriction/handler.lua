...
local function cidr_cache(cidr_tab)
  local parsed_cidrs = new_tab(#cidr_tab, 0) -- table of parsed cidrs to return

  -- build a tab of parsed cidr blocks based on configured
...
function IpRestrictionHandler:access(conf)
  ...
  if conf.blacklist and #conf.blacklist > 0 then
    block = iputils.binip_in_cidrs(remote_addr, cidr_cache(conf.blacklist))
  end

  if conf.whitelist and #conf.whitelist > 0 then
    block = not iputils.binip_in_cidrs(remote_addr, cidr_cache(conf.whitelist))
  end
